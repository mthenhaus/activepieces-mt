"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _chunkBSMIECIGjs = require('./chunk-BSMIECIG.js');


var _chunkXXPGZHWZjs = require('./chunk-XXPGZHWZ.js');

// src/secp256r1/public.ts
var _p256 = require('@noble/curves/p256');
var SECP256R1PublicKeyLength = 33;
var SECP256R1PublicKey = class {
  constructor(bytes) {
    this.bytes = bytes;
    _chunkXXPGZHWZjs.__publicField.call(void 0, this, "curve", _chunkBSMIECIGjs.KindSECP256R1);
    if (this.bytes.length !== SECP256R1PublicKeyLength) {
      throw RangeError("bytes are not a valid secp256r1 public key");
    }
  }
  async verify(message, sig) {
    return _p256.secp256r1.verify(sig, message, this.bytes);
  }
};




exports.SECP256R1PublicKeyLength = SECP256R1PublicKeyLength; exports.SECP256R1PublicKey = SECP256R1PublicKey;
